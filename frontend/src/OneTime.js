...
const Onetime = () => {
    ...
    const handleSubmit = async (e) => {
		...
		setIsLoading(true);
        try {
          const { data: res } = await axios.post(`${Api_url}/Otp/Otp1`, { Otp});
...
<Grid item lg={12} sx={{justifyContent:'center',display:'flex',alignItems:'center'}}>
    <Paper sx={{ width: mobile?"85%":'478px', height: mobile?'300px':'336px',borderRadius:mobile?"16px":'26px',justifyContent:'center',display:'flex',alignItems:'center'}}>
      <Grid container lg={11} justifyContent="center" alignItems="center">
        <Grid item lg={10} xs={12} sx={{textAlign:'center'}}>
        <Typography sx={{fontSize:mobile?"20px":'26px',fontFamily:'Inter',fontWeight:'600'}}>One Time Password</Typography>
      </Grid>
