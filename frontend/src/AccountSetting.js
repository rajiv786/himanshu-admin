...
const handleTogglePasswordVisibility1 = () => {
  ...
  const [newPassword, setNewPassword] = useState('');
	...
	const photourl1='https://drive.google.com/uc?export=view&id=1oS9zbzWMrGgKbj-3SOyW-HYWHZ7FY8TY'
  ...
  const handleImageUpload = async (event) => {
    ...
    const formData = new FormData();
	  ...
	  setloading(true)
	  try {
		const response = await axios.post(`${Api_url}/Otp/users/photo`, formData, {
		  headers: {
  ...
  const handleChangePassword = async () => {
    if (newPassword === confirmPassword) {
      try {
        await axios.post(`${Api_url}/Otp/update-password`, { newPassword,user }); // Replace 123 with the actual user ID
        setNewPassword('');
        setConfirmPassword('');
  ...
  return (
	  <div>
		  <Helmet>
		  <meta name="title" content="My Account - Manage Your Slushie Profile" />
     
         
        <meta name="description" content="Access and manage your Slushie profile. Update your author information and preferences with ease. Your account, your way" />
      </Helmet>
		  ...
		  <Grid container lg={10} xs={10} sx={{ margin: 'auto' }}>
			  ...
			  <Typography sx={{fontSize:mobile?'26px':'36px',fontFamily:'Inter',fontWeight:mobile?500:700,color:"#1E1E1E"}}>My Account</Typography>  
			  </Grid>
			 
		  </Grid>
		  <ToastContainer />
		  <Grid container lg={10} xs={10} sx={{ margin: 'auto', borderRadius: '12px', backgroundColor: '#FAF7F7' }}>
		  ...
		  <Grid container lg={2.5} sx={{justifyContent:mobile?"center":"",marginTop:mobile?'36px':'0px'}}>
				  ...
				  {imageselect ? <img src={croppedImage?croppedImage:photourl1} alt="Profile52" style={{width:mobile?"64px":'188px',height:mobile?"64px":'188px',borderRadius:mobile?"64px":'188px'}} />:<img src={photoUrl?photoUrl:photourl1} alt="Profilephoto" style={{width:mobile?"64px":'188px',height:mobile?"64px":'188px',borderRadius:mobile?"64px":'188px'}} />}  
    ...
    <DialogActions>
      ...
      <Button onClick={saveCroppedImage} variant="contained" color="primary">
          ...
          {/* {croppedImage && (
            <div>
              ...
              <img src={croppedImage} alt="Cropped" />
					  ...
					  <Grid item lg={12} xs={12} sx={{marginTop:mobile?"8px":'16px',marginBottom:mobile?"36px":'',marginLeft:mobile?"0px":'10px'}}>
			  <label htmlFor="file-input">
				  <Typography sx={{
						  color: '#07b0f5', lineHeight: "24px", fontSize:'16px',textAlign:mobile?'center':''}}>
			  ...
			  <Grid container lg={9.5} xs={12} justifyContent="center" sx={{ marginTop: 'auto', marginBottom: 'auto' }}>
     				  <Grid item lg={10}  xs={12} sx={{marginRight:mobile?"0px":"56px",marginLeft:mobile?'16px':'0px'}}>
       ...
       disabled    />
					  ...
					  <Grid item lg={10}  xs={12} sx={{marginTop:mobile?'12px':'',marginLeft:mobile?'16px':'0px',marginRight:mobile?"0px":"56px",marginTop:mobile?'12px':'36px'}}>
          ...
          />
					  ...
					  <Grid item lg={10}  xs={12} sx={{marginRight:mobile?"0px":"56px",marginTop:mobile?'12px':'36px',marginLeft:mobile?'16px':'0px'}}>
          ...
          <TextField
            ...
            onChange={(e) => setUser({ ...user, name: e.target.value })}
						  ...
						  <Typography onClick={handleOpenDialog} sx={{
							  textAlign: "end",fontFamily: 'Inter',fontSize: '14px',color:'#07b0f5'}}>Change Password</Typography>
					  </Grid>
         ...
         <Typography  style={{fontFamily: 'Inter',fontSize: '16px',fontWeight:"700",textAlign:'initial'}}>Password</Typography>
          ...
          <TextField
            ...
            onChange={(e) => setNewPassword(e.target.value)}
								  />
								  <Button onClick={handleChangePassword} variant='contained' sx={{background:'#07b0f5',textTransform:'none',width:mobile?"71%":'366px',marginTop:'26px'}} color="primary">
            Change Password
          </Button>
        </DialogContent>
				  ...
				  <Grid container lg={12} xs={12} sx={{justifyContent:'end',display:'flex',marginTop:mobile?"57px":'36px',marginBottom:mobile?'36px':'46px'}}>
						  ...
						  <Button type="submit" style={{ color: "#07b0f5", border: '1px solid #07b0f5', textTransform: 'none',width:'53px' ,borderRadius:'8px'}} onClick={handleSubmit}>
            Cancel
						  </Button>  
						  </Grid>
						  <Grid item lg={1.8} xs={6.5} sx={{justifyContent:'end',display:'flex'}}>
						  <Button type="submit" style={{ backgroundColor: "#07b0f5",textTransform:'none',borderRadius:"8px"}} variant="contained" onClick={handleImageUpload}>
