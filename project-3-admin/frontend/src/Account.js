...
const handleTogglePasswordVisibility1 = () => {
  ...
  const handleChangePassword = async () => {
    if (newPassword === confirmPassword) {
      try {
        await axios.post(`${Api_url}/admin/update-password`, { newPassword,user }); 
        setNewPassword('');
        setConfirmPassword('');
	...
	<div>
		...
		<Grid container lg={2.5}>
			...
			<label htmlFor="file-input">
				<Typography sx={{
						color: '#07b0f5', lineHeight: "24px", fontSize:'16px'}}>
	 ...
	 disabled    />
	  ...
	  <DialogContent style={{width:"366px",height:"208px",}}>
	   <Typography  style={{fontFamily: 'Inter',fontSize: '16px',fontWeight:"700",textAlign:'initial'}}>Password</Typography>
		...
		type="submit"
		variant="contained"
		sx={{background:'#07b0f5',textTransform:'none',width:'366px',marginTop:'26px'}}
	  >
		{isLoading ? <CircularProgress style={{ color: "#FFFFFF" }} />: 'Change Password'}      
				...
				<Grid container lg={12} sx={{justifyContent:'end',display:'flex',marginTop:'76px'}}>
						...
						<Button type="submit" style={{ color: "#07b0f5", border: '1px solid #07b0f5', textTransform: 'none',width:'53px' }} onClick={handleSubmit}>
		  Cancel
						</Button>  
						</Grid>
