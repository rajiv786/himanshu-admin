...
const upload = multer({
	limits: {
	  fieldSize: 20 * 1024 * 1024, // Increase the field size limit to accommodate larger files
	},
  });
...
router.get('/api/images/rejected', async (req, res) => {
	try {
	  const images = await File.find({ status: 'rejected' }); // Retrieve only rejected images
	  ...
	  const imageItems = images.map((image) => ({
		...
		imageData: image.imagePath.toString('base64'),
		userId: image.userId,
		status: image.status // No need to convert Buffer to Base64 as we are only fetching rejected images
	  }));
  
	  res.status(200).json(imageItems);
	} catch (error) {
	  console.error('Error fetching rejected images:', error);
	  res.status(500).json({ error: 'Error fetching rejected images' });
	}
  });
  router.get('/api/images/pending', async (req, res) => {
	try {
	  const images = await File.find({ status: 'pending' }); // Retrieve only rejected images
  ...
  router.get('/api/images/home', async (req, res) => {
	try {
	  const images = await File.find({ status: 'accepted' }); // Retrieve only accepted images
	  ...
	  const imageItems = images.map((image) => ({
		...
		imageData: image.imagePath.toString('base64'),
		userId: image.userId,
		status: image.status // No need to convert Buffer to Base64 as we are only fetching accepted images
	  }));
...
router.get('/api/images/homes', async (req, res) => {
	try {
      ...
      .limit(10); // Retrieve only accepted images
...
router.get('/api/images/home2', async (req, res) => {
	const allowedCategories = ['Algorithm', 'Fiction', 'Non Fiction'];
	try {
